<img width="299" height="24" alt="image" src="https://github.com/user-attachments/assets/01f2110b-4fe4-482d-9107-adaaded517c6" /># 🐶 UNIPET(유니펫) - 반려동물의 모든것!! 🐱

## 💡 프로젝트 소개

**UNIPET(유니펫)**은 반려동물을 키우는 가구의 증가와
관련 서비스에 대한 니즈에 맞춰
반려동물 관련 쇼핑, 병원(진료), 미용, 위탁(호텔/훈련), 커뮤니티를 
하나의 플랫폼에서 종합적으로 안내 및 연계하는 서비스를 제공하고 있습니다.

- 반려동물 용품 쇼핑 (주문, 결제, 배송, 리뷰, 쿠폰 및 포인트)
- 반려동물 서비스 병원(진료), 미용, 위탁(호텔/훈련) 업체 검색 및 예약, 결제, 서비스 완료 후 리뷰 작성
- 서비스 제공 업체는 업체 정보 등록 및 관리 (소개글, 사진, 예약 정책), 예약 및 리뷰 관리 
- (통합/지역) 커뮤니티를 통해 반려동물 관련 정보 공유 및 지역 내 커뮤니티 활동 가능

---

## 🗓 프로젝트 기간 (4/9 ~ 4/30 16일간)

- **4/9 ~ 4/14 (4일)** : 기획 및 설계 (테이블, 기능, 정책, 화면)  
- **4/15 ~ 4/28 (10일)** : 기능 개발 및 단위테스트  
- **4/29 ~ 4/30 (2일)** : 통합 테스트 및 디버깅

---

## 👨‍👩‍👦‍👦 팀원 구성

| 이름 | GitHub 프로필 | 이메일 |
|------|----------------|---------------|
| 김현동 | [Medo-skb](https://github.com/Medo-skb) | [kiryto0912@gmail.com](mailto:kiryto0912@gmail.com) |
| 장석재 | [Seokjae-Jang-git](https://github.com/Seokjae-Jang-git) | [tjrwowkd@gmail.com](mailto:tjrwowkd@gmail.com) |
| 이다빈 | [dabinah9526-ship-it](https://github.com/dabinah9526-ship-it) | [dabinah9526@gmail.com](mailto:dabinah9526@gmail.com) |
| 김예림 | [4288-yerim](https://github.com/4288-yerim) | [yryr4288@gmail.com](mailto:yryr4288@gmail.com) |
| 성기필 | [SeongGiPil](https://github.com/SeongGiPil) | [rlvf1234@naver.com](mailto:rlvf1234@naver.com) |

---

## 🛠 사용 기술

**Frontend**
<img src="https://img.shields.io/badge/Vue.js 3-4FC08D?style=for-the-badge&logo=vuedotjs&logoColor=white"> <img src="https://img.shields.io/badge/JSP-007396?style=for-the-badge&logo=java&logoColor=white"> <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"> <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white"> <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black"> <img src="https://img.shields.io/badge/jQuery-0769AD?style=for-the-badge&logo=jquery&logoColor=white">

**Backend**
<img src="https://img.shields.io/badge/Java 17-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"> <img src="https://img.shields.io/badge/SpringBoot 3-6DB33F?style=for-the-badge&logo=springboot&logoColor=white"> <img src="https://img.shields.io/badge/MyBatis-000000?style=for-the-badge&logo=mybatis&logoColor=white"> <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white"> <img src="https://img.shields.io/badge/AWS RDS-527FFF?style=for-the-badge&logo=amazonrds&logoColor=white">

**APIs & Tools**
<img src="https://img.shields.io/badge/Google Gemini-8E75C2?style=for-the-badge&logo=googlegemini&logoColor=white"> <img src="https://img.shields.io/badge/Kakao-FFCD00?style=for-the-badge&logo=kakaotalk&logoColor=black"> <img src="https://img.shields.io/badge/Naver-03C75A?style=for-the-badge&logo=naver&logoColor=white"> <img src="https://img.shields.io/badge/PortOne-F73131?style=for-the-badge&logo=paypay&logoColor=white"> <img src="https://img.shields.io/badge/Coolsms-0052CC?style=for-the-badge&logo=messagebird&logoColor=white"> <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"> <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white">

---

## 📕 발표 PPT  
<!-- [▶ 과실제로 발표 PPT](https://drive.google.com/file/d/16b62kuGLnBxYrV_PAOYX5HQSF9qK51Kx/view?usp=drive_link) -->

## 🎞 시연 영상  
<!-- [▶ 과실제로 핵심기능 시연 영상](https://drive.google.com/file/d/1erdU57Da71Rt3PvvUt7lkM8fbHMvL8Fa/view?usp=sharing) -->

[팀원별 페이지 시연 영상]

 [▶김현동]()
  
 [▶장석재]()
  
 [▶이다빈]()
  
 [▶김예림]()
  
 [▶성기필]()

---

## ⭐️ 주요 기능

-  예약
    - 내 위치 기반 주변 업체 조회 (Kakao 지도 API)
    - 병원/미용실/위탁시설 카테고리별 검색
    - 예약 희망하는 일자 및 시간에 맞춰 예약 가능한 업체 검색하여 조회
    - 유니펫 회원사를 목록 상단에 노출
    - 예약 가능 일자 및 시간 조회 (불가는 일자 및 시간 비활성화)
    - 등록한 반려동물 선택, 진료/서비스 선택하여 예약 및 결제 (예약금은 진료/서비스 금액의 10%)

-  쇼핑
    - 카테고리 및 필터 세분화를 통한 상품 검색 용이
    - 동물 (강아지, 고양이, 조류, 어류, 기타) -> (강아지_소형견, 중형견, 대형견)
    - 상품 (식품/영양, 홈/케어, 패션/외출, 놀이/훈련) -> (식품/영양_사료/주식, 간식, 영양제)
    - 동물+상품 필터 조합 가능 (예: 소형견 + 간식)
    - 상품 정렬 기능 (최신순, 가격순, 리뷰수순, 평점순)
    - 상품별 추천 대상 안내 (예: 단모종을 위한 상품)
    - 장바구니, 결제, 환불 기능 제공

-  커뮤니티
    - 통합 (일반, 정보/팁, 움짤, 동영상, 질문, 나눔/장터) 게시판 제공
    - 지역 (서울, 인천, 경기 ~ 제주) (산책, 소모임, 여행, 지역정보, 병원추천, 돌봄, 중고거래) 게시판 제공
    - 게시글 정렬 기능 (최신순, 추천순, 조회순, 댓글순)
    - 게시글 임시저장 및 최근임시저장 불러오기, 파일첨부, 비공개글 작성 가능
    - 임시저장 목록 관리 가능
    - 알림 기능 (댓글, 추천 알림)
    - 부적절한 게시글 및 댓글 신고 가능

- 마이페이지 (사용자)
  - 사용자가 이용하는 모든 서비스 정보 조회 및 관리 가능
    - 홈, 구독, 커뮤니티, 주문내역, 예약내역, 반려동물, 포인트, 쿠폰 페이지
    - 홈: 마이페이지 대쉬보드 (내 프로필, 반려동물 프로필, 주문내역 등)
    - 주문내역: 주문 상세 정보 확인 및 제품 리뷰 작성 가능
    - 예약내역: 예약 상세 정보 확인 및 리뷰 작성 가능, 예약 신청 후 결제 미결(예약 대기상태)된 예약 결제 가능
    - 포인트: 포인트 보유 현황 및 사용내역 조회 가능
    - 쿠폰: 쿠폰 보유 현황 (사용가능/완료/만료) 조회 가능
    
- 마이페이지 (사업자)
  - 사업자가 제공하는 서비스 관리 가능
    - 홈, 정보수정, 예약 현황, 리뷰 관리
    - 홈: 마이페이지 대쉬보드 (오늘의 일정, 예약 비율, 예약 건수 그래프)
    - 정보수정: 상세 정보 (소개글, 이미지, 메뉴 등)수정 
    - 예약현황: 오늘 예약, 예약완료, 서비스완료 조회 가능
    - 리뷰관리: 사용자가 작성한 리뷰 조회 및 신고 가능, 전체 리뷰 조회 및 평균 평점 확인 가능
  
- 관리자
  - 신고 관리 (리뷰 및 커뮤니티 신고)
  - 사업자 입점 승인 관리: 사업자 입점 승인/반려 가능
  - 쇼핑몰 문의 답변 관리: 쇼핑몰 상품에 대한 문의 답변 작성 / 문의 삭제 가능
 
- 결제
  - 결제 페이지: 배송지 입력, 상품 확인, 포인트·쿠폰 적용, 결제수단 선택 가능
  - 결제 완료/실패 안내: 주문번호·결제금액·배송상태 안내 및 실패 사유/재결제 유도
  - 환불 페이지: 결제 후 환불 신청 및 환불 진행 가능

---

## 🧑‍💼 개발 담당 & 역할

### 김현동
- **주요 페이지**: 결제 페이지
- **기능 구현 & 역할**
  - 기능 구현: (외부 결제 API 이용) 쇼핑/예약/구독 결제, 환불, 포인트 및 쿠폰, 프로젝트 DB테이블 설계
  - 역할 : 팀장(회의 진행, 구글 드라이브 관리, 깃허브 관리, 개발 조율 및 관리), 시스템 테스트 및 이슈 등록, 발표자료 (설계) 작성

### 장석재
- **주요 페이지**: 예약 페이지, 리뷰 페이지
- **기능 구현 & 역할**
  - 기능 구현: (외부 지도 API 이용) 업체 조회, 예약, 예약 DB 테이블 설계
  - 역할 : 회의록 작성, 시스템 테스트 및 이슈 관리, 발표자료 (Readme & ppt) 작성

### 이다빈
- **주요 페이지**: 쇼핑 페이지, 커뮤니티 페이지
- **기능 구현 & 역할**
  - 기능 구현: 쇼핑몰 상품 등록 및 조회, 상품 상페 페이지, 장바구니, 커뮤니티 게시판/게시글 등록 및 조회 
  - 역할 : 발표자료 (Readme & ppt) 작성
  
### 김예림
- **주요 페이지**: 메인 페이지, 검색 페이지, 마이 페이지(사업자)
- **기능 구현 & 역할**
  - 기능 구현: 베너, 추천/인기 업체 및 상품 조회, 전체 CSS 적용
  - 역할 : 시스템 테스트 및 이슈 등록, 발표자료 (기능구현 영상 편집)

### 성기필
- **주요 페이지**: 회원가입, 로그인, 마이 페이지 (사용자)
- **기능 구현 & 역할**
  - 기능 구현: 회원가입, 로그인 (사용자/사업자/관리자), (외부 API 이용) 간편로그인, 마이페이지 관련 데이터 조회 및 등록
  - 역할 : 회의록 작성, 발표자료 (설계) 작성

---

## 📫 문의

프로젝트에 대한 문의는 [김현동_팀장](mailto:kiryto0912@gmail.com)에게 연락주세요.

---

## 📂 프로젝트 자료 모음

| 분류 | 링크 |
|------|------|
| 📝 회의록 | [회의록 보기](https://drive.google.com/drive/folders/1d1CxrihnG_TbVloRdNIfcGgQ-Lkdas0k?usp=drive_link) |
| 🏗 설계 자료 | [설계 보기](https://drive.google.com/drive/folders/17cAWy-taTVDPN2uJo4NCYHvQSAF1TFwI?usp=drive_link) |
| 📚 공통 문서 | [공통 문서 보기](https://drive.google.com/drive/folders/1k4MLxfWRZGS_JbnkDp5hF_ht_NbGSbwh?usp=drive_link) |
| 🛠 수정 내역 | [수정 로그 보기](https://drive.google.com/drive/folders/1cya6UYHxQlYqgekYmhSyMF3aRj55SYAP)

---

## 📂 프로젝트 구조도

```
Unipet/
├── pom.xml                               	# Maven 라이브러리 및 빌드 설정
├── src/
│   ├── main/
│   │   ├── java/com/example/unipet/
│   │   │   ├── common/                 	# 공통 설정 및 유틸리티
│   │   │   ├── config/                   	# 보안 및 환경 설정
│   │   │   ├── controller/            	    # 컨트롤러 (웹 요청 처리)
│   │   │   ├── dao/                      	# 서비스 (로직)
│   │   │   ├── mapper/                   	# MyBatis 매퍼 인터페이스
│   │   │   ├── model/                    	# DB 테이블과 매핑되는 DTO/VO
│   │   │   └── scheduler/                	# 예약, 구독 관련 스케줄러
│   │   │
│   │   ├── resources/
│   │   │   ├── application.properties    	# DB 연동 및 스프링 부트 전역 설정 파일
│   │   │   └── mybatis-mapper/           	# SQL 쿼리가 작성된 XML 파일 (sql-*.xml)
│   │   │
│   │   └── webapp/                    
│   │       ├── css/                      	# 전역 CSS 파일
│   │       ├── img/                      	# 각종 이미지 리소스
│   │       ├── js/                       	# 전역 및 메인 페이지 스크립트 (main.js 등)
│   │       └── WEB-INF/                  
│   │           ├── default.jsp           	# 기본 레이아웃 템플릿
│   │           ├── header/               	# 헤더 JSP
│   │           ├── footer/               	# 푸터 JSP
│   │           ├── main/                 	# 메인 화면 및 Gemini 챗봇 화면
│   │           ├── user/                 	# 일반 유저 기능 (회원가입, 로그인, 마이페이지, 리뷰)
│   │           ├── bizMyPage/            	# 비즈니스 유저 마이페이지 
│   │           ├── admin/                	# 관리자 전용 페이지
│   │           ├── product/              	# 쇼핑몰 상품 상세 및 장바구니
│   │           ├── payment/              	# 예약 및 쇼핑 결제, 환불 화면
│   │           ├── reservation/          	# 예약 검색, 진행 및 완료 화면
│   │           └── board/                	# 커뮤니티 게시판 (목록, 글쓰기, 상세보기)
│   └── test/                             	# 테스트 코드 영역
```
