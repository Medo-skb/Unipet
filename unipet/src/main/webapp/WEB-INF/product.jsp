<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
	...
	<body>
		<div id="app">
			<header>
				<img src="../img/유니펫 로고 디자인.png" class="logo" @click="fnReload">
			</header>
			...
			<div class="container">
				<div class="recommend-banner">
					...
					<div class="recommend-grid">
						<div v-for="item in recommendList" :key="item.productNo" class="product-card"
							@click="fnDetail(item.productNo)">
							<div class="img-box"><img :src="'/upload/'+item.fileName"
									onerror="this.src='https://via.placeholder.com/200'"></div>
							<div class="info-box">
								<div class="p-brand">{{item.brand}}</div>
								<div class="p-name">{{item.productName}}</div>
								<div class="p-price">{{item.productPrice.toLocaleString()}}원</div>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="search-area">
					...
					<main class="main-content">
						...
						<div v-if="list.length == 0" style="text-align:center; padding:50px; color:#999;">검색 결과가 없습니다.
						</div>
					</main>
				</div>
			</div>
		...
		<script>
			const app = Vue.createApp({
				data() {
					return {
						recommendList: [], list: [],
						animalMap: {}, productMap: {},
						animalMain: '', productMain: '',
						aSubNo: '', iSubNo: '',
						keyword: '', sortType: 'new'
					}
				},
				methods: {
					// 카테고리 데이터 로드
					...
					fnSetAnimal(main, subNo) {
						...
						this.fnList(); // 필터링된 리스트 호출
					},
					// 상품 카테고리 선택
					fnSetProduct(main, subNo) {
						this.aSubNo = ''; // 반려동물 카테고리 필터 해제
						if (subNo === '') {
					...
					// 필터 초기화
					fnResetFilters() {
						this.animalMain = ''; this.productMain = '';
						this.aSubNo = ''; this.iSubNo = '';
						this.fnList();
					},
					// 상품 리스트 호출
					fnList() {
						...
						$.ajax({
							url: "/list.dox", // 상품 리스트를 가져오는 컨트롤러 주소
							type: "POST",
					...
					fnGetRecommend() {
						let self = this;
						$.ajax({
							url: "/list.dox", type: "POST", contentType: "application/json",
							data: JSON.stringify({isBest: 'Y'}),
							success: function (data) {self.recommendList = data.list.slice(0, 4);}
						});
					},
					fnDetail(pNo) {
						// 상품 번호를 쿼리 스트링으로 들고 이동
						location.href = "/product/view.do?productNo=" + pNo;
					}
					fnSort(type) {this.sortType = type; this.fnList();},
					fnDetail(pNo) {location.href = "/view.do?productNo=" + pNo;},
					fnReload() {location.href = "/main.do";}
				},
				mounted() {
